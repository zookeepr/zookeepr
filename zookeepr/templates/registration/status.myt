...
% #endif
      <div class = "graph-bar" style = "width:<%c.ceilings['conference'].percent_sold() %>%">&nbsp;</div>
      <div class = "graph-bar-text"><% c.ceilings['conference'].percent_sold() %></div><br>
% if c.ceilings['earlybird'].available():
      <b>Earlybird</b> is available<br><br>
      <div class = "graph-bar" style = "width:<%c.ceilings['earlybird'].percent_sold() %>%">&nbsp;</div>
      <div class = "graph-bar-text"><% c.ceilings['earlybird'].percent_sold() |h%></div><br>
% else:
...
% if not 'signed_in_person_id' in session:
    ...
    (you've already registered, submitted a proposal or similar).  If you can't
...
%   session.save()
    ...
    <p><a href="/registration/new">Go directly to the registration form</a> otherwise.</p>
...
% elif False and c.signed_in_person and not c.signed_in_person.registration.volunteer:
    ...
    <p><a href="/registration/<% c.signed_in_person.registration.id %>/volunteer">Select areas of interest and ability</a></p>

    <h3>Other option</h3>

    <p><a href="/registration/<% c.signed_in_person.registration.id %>/edit">Edit details</a></p>
...
% elif not c.signed_in_person.invoices:
    ...
    <p><a href="/registration/<% c.signed_in_person.registration.id %>/pay">Generate invoice</a></p>

    <h3>Other option</h3>

    <p><a href="/registration/<% c.signed_in_person.registration.id %>/edit">Edit details</a>
%   if False and c.signed_in_person.registration.type=='Volunteer':
      <br><a href="/registration/<% c.signed_in_person.registration.id %>/volunteer">Change areas of interest and ability</a>
%   #endif
    </p>
...
%   if c.signed_in_person.registration.type=='Volunteer':
    <a href="/registration/<% c.signed_in_person.registration.id %>/volunteer">Change areas of interest and ability</a><br>
%   #endif
%   if not c.signed_in_person.invoices[0].good_payments:
    <a href="/registration/<% c.signed_in_person.registration.id %>/edit">Edit details</a><br>
    <a href="/registration/<% c.signed_in_person.registration.id %>/pay">Regenerate invoice</a><br>
%   else:
    <a href="/registration/<% c.signed_in_person.registration.id %>">View details</a><br>
%   #endif
    <a href="/invoice/<% c.signed_in_person.invoices[0].id %>" >View invoice</a><br>
    <a href="/invoice/<% c.signed_in_person.invoices[0].id %>/printable" >View printable invoice (html)</a><br>
    <a href="/invoice/<% c.signed_in_person.invoices[0].id %>.pdf" >Get printable invoice (pdf)</a></p>
...
% else:
    ...
    <p><a href="/invoice/<% c.signed_in_person.invoices[0].id %>/pay" >Pay your registration</a></p>

    <h3>Other options</h3>

    <p><a href="/registration/<% c.signed_in_person.registration.id %>/edit">Edit details</a>
    <br><a href="/registration/<% c.signed_in_person.registration.id %>/pay">Regenerate invoice</a>
    <br><a href="/invoice/<% c.signed_in_person.invoices[0].id %>" >View invoice</a>
    <br><a href="/invoice/<% c.signed_in_person.invoices[0].id %>/printable" >View printable invoice (html)</a>
    <br><a href="/invoice/<% c.signed_in_person.invoices[0].id %>.pdf" >View printable invoice (pdf)</a></p>
