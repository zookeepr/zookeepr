...
%           if c.signed_in_person.volunteer.accepted is None:
          Your application is currently being reviewed and we will contact you shortly with the outcome.
%           elif c.signed_in_person.volunteer.accepted:
          Your application has been accepted and we would like to invite you to register using the <% h.link_to('registration form', url=h.url(action='new')) %>
%           elif not c.signed_in_person.volunteer.accepted:
          Unfortunately your application to volunteer has not been accepted. If you would still like to register to attend the conference as a delegate please fill out the <% h.link_to('registration form', url=h.url(action='new')) %>.
%           #endif
        </p>
%       #endif
%   #endif
