...
<strong>firstname,lastname,email_address,invoice_id,profile_id,status</strong><br>
% count = 0
% for i in invoices:
%     speaker = False
...
<strong>
%     # endif
"<% i.person.firstname %>","<% i.person.lastname %>","<% i.person.email_address %>",<% i.id %>,<% i.person.id %>,<% status %><br>
%     if status == "INVALID":
</strong>
%     # endif
%     count += 1
